...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mt-4 bg-white rounded-xl shadow p-6 mb-6">
    ...
    <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white rounded-lg shadow p-4">
            <div class="text-sm text-gray-500">Сообщений всего</div>
            ...
            <div class="space-y-2">
                @forelse($chats as $chat)
                    <div class="border-b border-gray-100 pb-2 text-sm">
                        ...
                        <div class="text-xs text-gray-500">{{ $chat->created_at->format('d.m.Y H:i') }}</div>
                    </div>
                @empty
                    <p class="text-gray-500 text-sm">Чатов нет</p>
                @endforelse
            </div>
        </div>
            ...
            <div class="space-y-2">
                @forelse($payments as $p)
                    <div class="border-b border-gray-100 pb-2 text-sm flex justify-between">
                        ...
                        <span class="px-2 py-1 text-xs rounded-full {{ $p->status === 'CONFIRMED' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-700' }}">
                            {{ $p->status }}
                        </span>
                    </div>
                @empty
                    <p class="text-gray-500 text-sm">Платежей нет</p>
                @endforelse
            </div>
        </div>
    </div>
