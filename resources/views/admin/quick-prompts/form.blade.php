...
<div class="max-w-xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <h1 class="text-2xl font-bold text-gray-900 mt-4 mb-6">{{ $prompt ? 'Редактировать промпт' : 'Новый промпт' }}</h1>

    <form method="POST" action="{{ $prompt ? route('admin.quick-prompts.update', $prompt) : route('admin.quick-prompts.store') }}" class="bg-white shadow rounded-lg p-6 space-y-4">
        @csrf
        @if($prompt) @method('PUT') @endif

        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Иконка (эмодзи)</label>
            <input type="text" name="icon" value="{{ old('icon', $prompt?->icon ?? '📄') }}" required class="w-full border border-gray-300 rounded px-3 py-2 text-2xl text-center" maxlength="10">
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Название</label>
            <input type="text" name="title" value="{{ old('title', $prompt?->title) }}" required class="w-full border border-gray-300 rounded px-3 py-2" placeholder="Претензия в магазин">
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Ключ (латиница, для кода)</label>
            <input type="text" name="key" value="{{ old('key', $prompt?->key) }}" required pattern="[a-z0-9_]+" class="w-full border border-gray-300 rounded px-3 py-2 font-mono" placeholder="claim_shop">
            <p class="text-xs text-gray-500 mt-1">Только строчные буквы, цифры и подчёркивания</p>
        </div>
